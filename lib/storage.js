...
var S3Handler = {
  options: {
    ...
    S3MboxBucketSuffix: '.ses.inbound'
    //S3KeyName: 'mbox.json'
  },
  ...
  refreshINBOX: function(callback) {
    if (CONSOLE_MESSAGES) console.log(new Date().toISOString(),'S3Handler.refreshINBOX called');
    
    if (typeof S3Handler.options.user == 'undefined') {
      callback('Unable to refresh INBOX: No user defined'); // no user set, bail
    }

    S3Handler.options.mboxName = S3Handler.options.user.replace('@','--');

    if (typeof S3Handler.options.bucket == 'undefined') S3Handler.options.bucket = S3Handler.options.mboxName + S3Handler.options.S3MboxBucketSuffix;
    if (CONSOLE_MESSAGES) console.log(new Date().toISOString(),'bucket:',S3Handler.options.bucket);

    var S3 = S3Handler.s3; // shortcut local var
    ...
    S3.getObject({
      ...
      if (err) {
        ...
        data = JSON.parse(mbox.Body);
      }
      // track last message loaded
      // marker is used when doing a S3 listObjects so is no longer required
  ...
  /**
   * Wrapper for S3 getObject
   * Used by: mailbox.getMessageRange()
   */
  getObject: function(params, cb) {
    if (CONSOLE_MESSAGES) console.log(new Date().toISOString(),'S3Handler.getObject params:',params);

    S3Handler.s3.getObject({Bucket: S3Handler.options.bucket, Key: params.Key}, function(error, responseData) {
      if (error) {
        if (CONSOLE_MESSAGES) console.log(new Date().toISOString(),"S3.getObject Error:", error);
        cb(error, null);
      } else {
        ...
        if (CONSOLE_MESSAGES) console.log(new Date().toISOString(),'S3Handler.getObject responseData:',responseData);
...
getMailbox = function(path) {
    if (CONSOLE_MESSAGES) console.log(new Date().toISOString(),'getMailbox called');
...
getMessageRange = function (messages,range, isUid) {
  if (CONSOLE_MESSAGES) console.log(new Date().toISOString(),'getMessageRange called:',messages,range, isUid);
  ...
  if (CONSOLE_MESSAGES) console.log(new Date().toISOString(),'getMessageRange result:',result);
	return result;
}, // getMessageRange

search = function (folder,query) {
	if (CONSOLE_MESSAGES) console.log(new Date().toISOString(),'search called');
...
processMailbox = function(path, mailbox, namespace) {
    if (CONSOLE_MESSAGES) console.log(new Date().toISOString(),'processMailbox called:',path,mailbox,namespace);
...
processMessage = function(message, mailbox) {
    if (CONSOLE_MESSAGES) console.log(new Date().toISOString(),'processMessage called');
...
indexFolders = function() {
    if (CONSOLE_MESSAGES) console.log(new Date().toISOString(),'indexFolders called');
...
makeMessage = function () {
	if (CONSOLE_MESSAGES) console.log(new Date().toISOString(),'makeMessage called');
...
makeFolder = function (f) {
	if (CONSOLE_MESSAGES) console.log(new Date().toISOString(),'makeFolder called:',f);
...
makeMailbox = function () {
	return {
		folders: function (cb) {
			if (CONSOLE_MESSAGES) console.log(new Date().toISOString(),'mailbox/folders called');

      cb(null,_.keys(data));
		},
		getFolder: function (folder,cb) {
			if (CONSOLE_MESSAGES) console.log(new Date().toISOString(),'mailbox/getFolder called:', folder);
      ...
      if (folderCache[folder]) {
	    ...
	    if (CONSOLE_MESSAGES) console.log(new Date().toISOString(),'mailbox/createFolder called');
      ...
      // detect namespace for the path
		...
		delFolder: function (path,cb) {
			if (CONSOLE_MESSAGES) console.log(new Date().toISOString(),'mailbox/delFolder called');
      ...
      var that = this;
		...
		renameFolder: function (source,destination,cb) {
			if (CONSOLE_MESSAGES) console.log(new Date().toISOString(),'mailbox/renameFolder called:',source,destination);
      ...
      var that = this;
	    ...
	    if (CONSOLE_MESSAGES) console.log(new Date().toISOString(),'mailbox/createMessage called');
      ...
      var message = {
		...
		addFlags : function (folder,messages,isUid,flags,cb) {
			if (CONSOLE_MESSAGES) console.log(new Date().toISOString(),'mailbox/addFlags called');
      ...
      var ret = [];
		...
		removeFlags : function (folder,messages,isUid,flags,cb) {
			if (CONSOLE_MESSAGES) console.log(new Date().toISOString(),'mailbox/removeFlags called');
      ...
      var ret = [];
		...
		replaceFlags : function (folder,messages,isUid,flags,cb) {
			if (CONSOLE_MESSAGES) console.log(new Date().toISOString(),'mailbox/replaceFlags called');
      ...
      var ret = [];
		...
		addProperties : function (folder,messages,isUid,properties,cb) {
			if (CONSOLE_MESSAGES) console.log(new Date().toISOString(),'mailbox/addProperties called');
      ...
      var ret = [];
		...
		removeProperties : function (folder,messages,isUid,properties,cb) {
			if (CONSOLE_MESSAGES) console.log(new Date().toISOString(),'mailbox/removeProperties called');
      ...
      var ret = [];
		...
		replaceProperties : function (folder,messages,isUid,properties,cb) {
			if (CONSOLE_MESSAGES) console.log(new Date().toISOString(),'mailbox/replaceProperties called');
      ...
      // check that the folder exists
		...
		listMessages: function (folder,cb) {
			if (CONSOLE_MESSAGES) console.log(new Date().toISOString(),'mailbox/listMessages called');

      cb();
		},
		searchMessages: function (folder,query,cb) {
			if (CONSOLE_MESSAGES) console.log(new Date().toISOString(),'mailbox/searchMessages called');
      ...
      var results = search(folder,query);
		...
		getMessages: function (folder,msg,cb) {
			if (CONSOLE_MESSAGES) console.log(new Date().toISOString(),'mailbox/getMessages called:',folder,msg);

      cb(null,makeMessage());
		},
		getMessageRange: function (folder, range, isUid, cb) {
			if (CONSOLE_MESSAGES) console.log(new Date().toISOString(),'mailbox/getMessageRange called:',folder, range, isUid);
      ...
      async.forEachOf(results, function(item, index, callback) {
        ...
        if (CONSOLE_MESSAGES) console.log(new Date().toISOString(),'async.forEachOf:',item,index);
        ...
        if (err) {
          ...
          if (CONSOLE_MESSAGES) console.log(new Date().toISOString(),'Altered results:',results);
		...
		matchFolders : function(reference, match, callback) {
		    if (CONSOLE_MESSAGES) console.log(new Date().toISOString(),'mailbox/matchFolders called');
		...
		subscribeFolder: function (path,callback) {
			if (CONSOLE_MESSAGES) console.log(new Date().toISOString(),'mailbox/subscribeFolder called');
      ...
      if (!folderCache[path] || folderCache[path].flags.indexOf("\\Noselect") >= 0) {
		...
		setFolderSpecialUse: function (path,attrs,callback) {
			if (CONSOLE_MESSAGES) console.log(new Date().toISOString(),'mailbox/setFolderSpecialUse called');
      ...
      if (folderCache[path]) {
		...
		expunge: function (folder, cb) {
		  if (CONSOLE_MESSAGES) console.log(new Date().toISOString(),'mailbox/expunge called');
      ...
      var deleted = [],
		...
		list: function (cb) {
			if (CONSOLE_MESSAGES) console.log(new Date().toISOString(),'mailbox/list called');

      cb(null,["INBOX"]);
		},
		getNamespaces: function (cb) {
			if (CONSOLE_MESSAGES) console.log(new Date().toISOString(),'mailbox/getNamespaces called');
      ...
      var result = {personal: [], other: [], shared:[]};
		...
		namespace: function (path,cb) {
	  	if (CONSOLE_MESSAGES) console.log(new Date().toISOString(),'mailbox/namespace called');

      cb(null,data[path || referenceNamespace]);
		},
		delMessage: function (folder,id,cb) {
			if (CONSOLE_MESSAGES) console.log(new Date().toISOString(),'mailbox/delMessage called');

      cb();
		},
		moveMessage: function (folder,id,cb) {
			if (CONSOLE_MESSAGES) console.log(new Date().toISOString(),'mailbox/moveMessage called');

      cb();
		},
		readMessage: function (folder,id,read,cb) {
			if (CONSOLE_MESSAGES) console.log(new Date().toISOString(),'mailbox/readMessage called');

      cb();
		},
		starMessage: function (folder,id,star,cb) {
			if (CONSOLE_MESSAGES) console.log(new Date().toISOString(),'mailbox/starMessage called');
...
module.exports = function (s) {
	...
	return {
      ...
      S3Handler.options.bucket = user.replace('@','--') + S3Handler.options.S3MboxBucketSuffix;
...
module.exports.setConsoleMessages = function(value) {
  CONSOLE_MESSAGES = value || false;
};
